...
The bet class we will need to be able to keep track of the number of
 dice and the face value.  we should probably  create an array or
...
/**
 ...
 on the previous bet.  if the number of dice  in the current that is
 greater than the number of dice in the previous bet then the face value
 can be any number.   if the number of dice is equal to the previous bet
 then the face value must be higher  Then the previous bet. If the
 number of dice is less than the previous bet then the bet is invalid.
*/


/**
We need a message it to be called when a player is accused of lying.
 this method will compare the bet to the total dice on the table
 either by cycling through players and collecting the dice values to
 ...
 number of dice with a designated face on the table then the player
...
We will need a method that constantly checks if any players win flag is
 a 1 this method will show all dice and end the game. this
